...
//            VideoEntity videoEntity = videoDao.getVideo(videoUuid);
            if (videoEntity == null) {
                throw new VideoNotFoundException("VID-001", "Video found");
            }
            return videoEntity;
        } else
            throw new UnauthorizedException("ATH-001", "Unauthorized Access");
    }
    ...
    public VideoEntity updateVideo(final VideoEntity videoEntity, final String authorization) throws VideoNotFoundException, UnauthorizedException, UserNotSignedInException {
        ...
        if (userAuthTokenEntity == null) {
            throw new UserNotSignedInException("USR-001", "You are not Signed in, sign in first to update the details of the video");
        }
